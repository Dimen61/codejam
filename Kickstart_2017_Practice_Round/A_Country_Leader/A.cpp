...
inline int countChars(const string& S) {
	bs.reset();
	for (const char& c : S) {
		if (isalpha(c))
			bs.set(c, 1);
...
inline bool ____greater(const string& A, const string& B) {
	int ccA = countChars(A);
	int ccB = countChars(B);
	return ccA > ccB || (ccA == ccB && A < B);
}
